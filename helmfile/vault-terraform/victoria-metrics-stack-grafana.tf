resource "vault_identity_oidc_assignment" "grafana" {
  name = "grafana"
  group_ids = [
    vault_identity_group.this["monitoring-admins"].id,
    vault_identity_group.this["monitoring-editors"].id,
    vault_identity_group.this["monitoring-viewers"].id,
  ]
}
...
# Kept hand-written (two OIDC clients don't fit the ./service module).
# The backup-cluster role lives in victoria-metrics-stack-grafana-backup.tf.
